...
namespace AudioProcessing.Stream
{
    internal static partial class Native
    {
        ...
        [DllImport("portaudio")]
        [return: MarshalAs(UnmanagedType.I4)]
        public static extern ErrorCode Pa_CloseStream(nint stream);       // `PaStream *`
        ...
        [DllImport("portaudio")]
        [return: MarshalAs(UnmanagedType.I4)]
        public static extern ErrorCode Pa_StartStream(nint stream);       // `PaStream *`

        [DllImport("portaudio")]
        [return: MarshalAs(UnmanagedType.I4)]
        public static extern ErrorCode Pa_StopStream(nint stream);        // `PaStream *`

        [DllImport("portaudio")]
        [return: MarshalAs(UnmanagedType.I4)]
        public static extern ErrorCode Pa_AbortStream(nint stream);       // `PaStream *`

        [DllImport("portaudio")]
        [return: MarshalAs(UnmanagedType.I4)]
        public static extern ErrorCode Pa_IsStreamStopped(nint stream);   // `PaStream *`

        [DllImport("portaudio")]
        [return: MarshalAs(UnmanagedType.I4)]
        public static extern ErrorCode Pa_IsStreamActive(nint stream);    // `PaStream *`

        [DllImport("portaudio")]
        public static extern double Pa_GetStreamTime(nint stream);        // `PaStream *`

        [DllImport("portaudio")]
        public static extern double Pa_GetStreamCpuLoad(nint stream);     // `PaStream *`

        [DllImport("portaudio")]
        public static extern ErrorCode Pa_ReadStream(nint stream, float[] buffer, uint frames);

        [DllImport("portaudio")]
        public static extern int Pa_GetStreamReadAvailable(nint stream);
    }
    ...
    /// </summary>
    public class PortAudioBlockingAudioStream : IDisposable
    {
        ...
        /// </summary>
        public PortAudioBlockingAudioStream(
        ...
        )
        {
            ...
            streamCallback = callback != null ? new _NativeInterfacingCallback<Callback>(callback) : null;
            ...
            nint inParamsPtr = nint.Zero;
            nint outParamsPtr = nint.Zero;
            ...
            if (inParamsPtr != nint.Zero)
                Marshal.FreeHGlobal(inParamsPtr);
            if (outParamsPtr != nint.Zero)
                Marshal.FreeHGlobal(outParamsPtr);
        }

        /// <summary>
        /// Read up to sampleCount frames from the stream in blocking mode.
        /// </summary>
        /// <param name="framesToRead">Number of frames (per channel) requested.</param>
        /// <returns>Interleaved float array of length framesToRead * Channels or null if read failed</returns>
        public float[]? ReadSamples(int framesToRead)
        {
            int? channelCount = inputParameters?.channelCount;
            if (streamPtr == nint.Zero || !channelCount.HasValue)
                throw new InvalidOperationException("Input stream not initialized");

            // Allocate buffer for interleaved channels
            float[] buffer = new float[framesToRead * channelCount.Value];

            ErrorCode err = Native.Pa_ReadStream(streamPtr, buffer, (uint)framesToRead);
            if (err != 0)
            {
                return null;
            }                
        ...
        /// <returns></returns>
        public static UD GetUserData<UD>(nint userDataPtr) =>
